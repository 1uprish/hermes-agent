...
function PermissionRow({
  ...
  const canRequest = status === 'not-granted' || status === 'ask-when-used'
...
function SettingsPage({
  ...
  if (view === 'general') {
    return (
      <SettingsScaffold description="Choose how MacMan starts, looks, and stays available." title="General">
        <SettingsGroup title="Startup">
          ...
          <SettingRow action={<Select value="Keep running" />} description="Keep working when the main window is closed." title="When closing the window" />
        </SettingsGroup>
        <SettingsGroup title="Appearance & input">
          <SettingRow action={<Select value="System" />} description="Follow macOS, or choose light or dark." title="Appearance" />
          <SettingRow action={<Select value="English" />} description="Language used throughout MacMan." title="Language" />
        ...
        <SettingsGroup title="Updates">
          ...
          <SettingRow action={<button className="mm-button mm-button--small">Check now</button>} description="MacMan 0.21.1 · Stable channel" title="Software update" />
        </SettingsGroup>
      </SettingsScaffold>
    )
  }
  ...
  if (view === 'voice') {
    return (
      <SettingsScaffold description="Control when MacMan listens, speaks, and can be interrupted." title="Voice & Audio">
        <SettingsGroup title="Input">
          <SettingRow action={<Select value="System default" />} description="The microphone used for voice conversations." title="Microphone" />
        ...
        <SettingsGroup title="Output">
          <SettingRow action={<Select value="System default" />} description="Where spoken responses play." title="Speaker" />
          <SettingRow action={<Select value="Natural" />} description="Choose the speaking voice and preview it." title="Voice" />
  ...
  if (view === 'notifications') {
    return (
      <SettingsScaffold description="Decide when MacMan should get your attention." title="Notifications">
  ...
  if (view === 'connections') {
    return (
      <SettingsScaffold description="Connect the apps and accounts MacMan can work with." title="Connections">
        <SettingsGroup title="Apple apps">
          <ConnectionRow description="Send and read messages on this Mac. Requires Full Disk Access." name="Messages" status="Not connected" />
          <ConnectionRow description="Create events and check availability." name="Calendar" status="Ask when used" />
          <ConnectionRow description="Create and complete reminders." name="Reminders" status="Ask when used" />
        </SettingsGroup>
        <SettingsGroup title="Web & accounts">
          <ConnectionRow description="Use your signed-in browser for approved tasks." name="Browser" status="Not connected" />
          <ConnectionRow description="Connect an email account through the wrapper." name="Email" status="Not connected" />
          <ConnectionRow description="Add supported tools without giving them blanket access." name="More connections" status="Browse" />
        </SettingsGroup>
      </SettingsScaffold>
    )
  }
  ...
  if (view === 'privacy') {
    return (
      <SettingsScaffold description="Set clear limits on what MacMan may do and remember." title="Privacy & Safety">
        <SettingsGroup title="Action safety">
          <SettingRow action={<Select value="Ask for sensitive actions" />} description="Always confirm messages, purchases, deletions, account changes, and terminal commands." title="Approval level" />
          ...
          <SettingRow action={<button className="mm-button mm-button--small">Manage</button>} description="Apps and locations MacMan must never inspect or control." title="Excluded apps and folders" />
        </SettingsGroup>
        <SettingsGroup title="Data">
          <SettingRow action={<Select value="30 days" />} description="How long local task history and diagnostic context are retained." title="Activity history" />
  ...
  return (
    <SettingsScaffold description="Runtime visibility and maintenance controls for advanced users." title="Advanced">
      <SettingsGroup title="Runtime">
        ...
        />
        <SettingRow action={<button className="mm-button mm-button--small">Open logs</button>} description="Review local logs with secrets redacted." title="Diagnostic logs" />
      ...
      <SettingsGroup title="Data & recovery">
        <SettingRow action={<button className="mm-button mm-button--small">Export</button>} description="Save settings, connection metadata, and local history." title="Export MacMan data" />
        <SettingRow action={<button className="mm-button mm-button--small mm-button--danger">Reset</button>} description="Remove local settings and return to this setup screen." title="Reset MacMan" />
      </SettingsGroup>
    </SettingsScaffold>
  )
}

function SettingsScaffold({ children, description, title }: { children: ReactNode; description: string; title: string }) {
  return (
...
function Select({ value }: { value: string }) {
  return (
    <button className="mm-select" type="button">
      {value} <IconChevronRight aria-hidden size={15} stroke={1.8} />
    </button>
  )
}
...
function ConnectionRow({ description, name, status }: { description: string; name: string; status: string }) {
  return (
    <SettingRow
      action={
        <button className="mm-button mm-button--small mm-button--quiet" type="button">
          {status}
        </button>
      }
    ...
    <div className="mm-app">
      ...
      <main aria-label={activeLabel} className="mm-main">
        {activeView === 'setup' ? (
          ...
          <SettingsPage view={activeView} wrapperStatus={wrapperStatus} />
        )}
      </main>
    </div>
  )
