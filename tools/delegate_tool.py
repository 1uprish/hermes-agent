...
_DESCRIPTION_HEAD = (
    "Fan out work to parallel subagents. Use this when the job splits into SEVERAL independent pieces that can run "
    "at the same time (review 5 files, research 3 questions, fix N unrelated bugs). A single or linear task — one "
    "thing, or steps where each depends on the last — is yours to do directly with your own tools; wrapping it in a "
    "subagent only adds latency and loses context. Other valid uses: one reasoning-heavy side task whose "
    "intermediate output would flood your context.\n"
    "Not for: mechanical multi-step work with no reasoning -> execute_code; a single tool call -> call it directly; "
    "anything needing user interaction -> subagents cannot ask questions; durable work that must survive this "
    "session -> cronjob or terminal(background=True, notify=True) (/stop, /new, or process exit discards running "
    "subagents).\n\n"
    "Each `tasks` entry spawns one subagent in an isolated context (own conversation, terminal session, toolset); "
    "only its final summary returns to you. Dispatch returns immediately with live transcript paths; results re-enter "
    "the conversation as a new message after you END YOUR TURN (one message per call by default; see `group`). Finish "
    "whatever does not depend on them, give a one-line status, and stop — never wait or poll on transcripts, artifact "
    "files, or CI for a child. While children run, `action` (list/steer/stop) controls them live.\n\n"
    "RULES:\n"
