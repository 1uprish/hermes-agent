...
# pm extra installed on first use, per provider whose SDK lazy-installs.
...
def warm_tts_provider(tts_config: Optional[Dict[str, Any]] = None, provider: Optional[str] = None) -> Dict[str, Any]:
    ...
    if feature is not None:
        try:
            from pm import available as _pm_available, ensure_import as _pm_ensure_import
            if _pm_available(feature):
                result.update(warmed=True, action="cached")
            else:
                _pm_ensure_import(feature)
