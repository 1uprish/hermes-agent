...
- **Conflicts.** For native shared-venv plugins, an unsatisfiable dependency union fails loudly: the candidate plugin stays unenabled and unimported (the admission authority refuses before publishing config, reporting the plugin identity plus the resolver's reason, with a re-enable/retry path and a machine-readable pm receipt). Nothing disables plugins automatically today — there is no automatic bisect. A future automatic-disable decision, if one is added, must prefer the active memory provider over ordinary plugins.
