...
To keep the guard enabled but adopt a deliberately changed global default in
one step, **resnap** instead of disabling it. Resnapping refreshes an unpinned
job's stored snapshot to the current global resolution — the job stays
unpinned (it keeps tracking future changes), and the drift guard re-arms
...
The agent-facing `cronjob` tool accepts the same action
(`action=resnap job_id=<id>` or `action=resnap all=true`). Pinned axes and
`no_agent` script jobs are left untouched.
