...
def test_workspace_uv_preserves_managed_tool_and_interpreter(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(ensure, "uv", lambda **kwargs: ("managed/uv", {"UV_PYTHON": "managed/python"}))
    monkeypatch.setattr(workspace, "_generate_pyproject", lambda *args: (tmp_path, False))
