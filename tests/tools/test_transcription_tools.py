...
class TestRunCommandSttIdleTimeout:
    ...
    def test_stderr_progress_extends_beyond_timeout(self, tmp_path):
        ...
        result = _run_command_stt(
            ...
            # The idle window must dwarf process startup on every host: a
            # fresh Windows spawn can take far longer than 0.1s to emit its
            # first byte, which is startup latency, not an idle stall.
            timeout=2.0,
        )
